    ...
    [
        {
            name: "Mouna",
            age: 28,
            favoriteFoods: ["Pain au chocolat", "Panini"],
        },

        {
            name: "Zayneb",
            age: 30,
            favoriteFoods: ["Omlette", "Banane"],
        },
...
Person.find({ name: "Cyrine" }, (err, data) => {
    err
        ? console.log(err)
        : console.log("You find the person with name Cyrine");
});

//FindOne operation by favorite food

Person.findOne({ favoriteFoods: "Panini" }, (err, data) => {
    err ? console.log(err) : console.log(`${data.name} like Panini`);
});
    ...
    (err, data) => {
        if (err) {
            ...
            data.save((err, doc) => {
                err
                    ? console.log(err)
                    : console.log("Data was Updated" + doc.favoriteFoods);
            });
        }
    }
);
...
Person.remove({ name: "Mehdi" }, (err) => {
    err
        ? console.log(err)
        : console.log("All document with name Mehdi are removed");
});

//Find people who like Banane

Person.find({ favoriteFoods: "Manga" })
    .sort({ name: 1 })
    .limit(2)
    .select({ age: 0 })
    .exec((err, data) => {
